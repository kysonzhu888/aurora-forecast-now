...
function ageSecondsForTimestamp(value, nowMs) {
...
export function evaluateForecastHealth({
  ...
  normalMaxAgeSeconds,
  stormMaxAgeSeconds,
  maxScheduleGapSeconds,
  nowMs = Date.now(),
}) {
  const hasCache = Boolean(metadata);
  const ageSeconds = hasCache ? ageSecondsForMetadata(metadata, nowMs) : Number.POSITIVE_INFINITY;
  const maxAgeSeconds = metadata?.stormMode ? stormMaxAgeSeconds : normalMaxAgeSeconds;
  const dataFresh = hasCache && ageSeconds <= maxAgeSeconds;
  const scheduleAgeSeconds = scheduleState
    ? ageSecondsForTimestamp(scheduleState.checkedAt, nowMs)
    : Number.POSITIVE_INFINITY;
  const scheduleHealthy = Boolean(scheduleState)
    && scheduleState.ok !== false
    && scheduleState.status !== "error"
    && scheduleAgeSeconds <= maxScheduleGapSeconds;

  return {
    ok: dataFresh,
    status: dataFresh ? (scheduleHealthy ? "healthy" : "degraded") : "unhealthy",
    hasCache,
    dataFresh,
    scheduleHealthy,
    fallbackNeeded: !dataFresh,
    ageSeconds: Number.isFinite(ageSeconds) ? ageSeconds : null,
    maxAgeSeconds,
    scheduleAgeSeconds: Number.isFinite(scheduleAgeSeconds) ? scheduleAgeSeconds : null,
  };
}

export function scheduledRefreshDecision(
  metadata,
  alertInfo,
  normalMaxAgeSeconds,
  nowMs = Date.now(),
  refreshLeadSeconds = 0,
) {
  ...
  const refreshWindowAgeSeconds = Math.max(0, normalMaxAgeSeconds - refreshLeadSeconds);
  if (refreshLeadSeconds > 0 && ageSeconds >= refreshWindowAgeSeconds) {
    return { shouldRefresh: true, reason: "scheduled-normal-refresh-window", ageSeconds };
  }
...
export async function runScheduledRefresh({
  ...
  try {
    ...
    const decision = scheduledRefreshDecision(
      metadata,
      alertInfo,
      normalMaxAgeSeconds,
      nowMs,
